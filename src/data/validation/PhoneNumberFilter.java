...
public class PhoneNumberFilter extends DocumentFilter{
    ...
    private final int PHONE_SIZE = 12;
    ...
    public void replace(DocumentFilter.FilterBypass fb, int offset, int length, String str, 
      AttributeSet attrs) throws BadLocationException {
        ...
        if (fb.getDocument().getLength() == 2 || fb.getDocument().getLength() == 6) { this.insertString(fb, fb.getDocument().getLength(), "-", attrs);}
        
        if ((fb.getDocument().getLength() + str.length()
            - length) <= PHONE_SIZE && str.matches("^[0-9]")) {
